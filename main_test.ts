...
Deno.test("route#template", async (t) => {
  const r = route("https://jasonraimondi.com/posts/:slug");

  await t.step("success", () => {
    assertEquals(r.template, "https://jasonraimondi.com/posts/:slug");
  });
});

Deno.test("route#create", async (t) => {
  const r = route("https://jasonraimondi.com/posts/:slug/random/:id");
